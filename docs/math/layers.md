...
- 对于 MNIST 28x28 的小图尤其重要——没有 padding 的话两层 3x3 卷积就会从 28→26→24，损失大量边缘信息

### 为什么用奇数核（3x3）？

- 奇数核有明确的中心点，方便对齐和设计 padding（$\lfloor 3/2 \rfloor = 1$）
- 两个 3x3 的感受野等价于一个 5x5，但参数量更少（$2 \times 9 < 25$），且中间多了一层非线性
...
每次 2x2 MaxPool 将空间分辨率减半（28→14→7），后续层参数所需输入维度指数级减少。两层池化后将 28x28=784 维降到 7x7x64=3136 维但保留了 64 个高级特征通道。

**2. 局部平移不变性**

如果数字"3"平移 1 个像素，2x2 窗口内的最大值大概率不变。这使得分类对微小平移不敏感——证明：
...
每次池化后，下一层卷积的实际感受野翻倍。两层池化后，第 3 个 ConvBlock 的 3x3 卷积可覆盖原始图像的 ~8x8 区域。
